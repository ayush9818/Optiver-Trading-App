...
if __name__ == "__main__":
    ...
    model_training_tab, model_inferences_tab, show_data_tab = st.tabs(["Model Training", "Model Inferences", "Show Data"])
    with model_training_tab:        
        ...
        st.markdown(f"Date")
        train_date_input = st.text_input("", get_date_from_date_id(train_date_id, holidays=st.session_state['holiday_dates_2023']), key="train_date", disabled=True, label_visibility="collapsed")

        # Display the base model name input (disabled)
        st.markdown("Base Model Name")
        base_model_name_input = st.text_input("", f"optiver-{st.session_state['last_date_id']}", key="base_model_name", disabled=True, label_visibility="collapsed")
        
        train_button_clicked = st.button("Train")
        if train_button_clicked:
            # Prepare data for the API request
            model_train_data = {
                "model_id": st.session_state['last_model_id'], # ID of the initial model to be fine-tuned
            ...
            try:
                ...
                response = st.session_state['api_post_handler'].post(train_model_endpoint, model_train_data)
                st.session_state['training_success'] = True
                fetch_models()
            except requests.exceptions.HTTPError as err:
                st.session_state['training_success'] = False
                if err.response.status_code == 500:
                    st.error("Error 500: Internal Server Error occurred while starting the training process.")
                else:
                    st.error(f"An error occurred: {err}")
            
        # Display success message if training was successful
        if st.session_state['training_success']:
            st.success(f"The newly trained model {st.session_state['last_model_name']} has been successfully uploaded to S3.")
    ...
    with model_inferences_tab:
        ...
        st.markdown(f"Date")
        prediction_date_input = st.text_input("", get_date_from_date_id(train_date_id, holidays=st.session_state['holiday_dates_2023']), key="prediction_date", disabled=True, label_visibility="collapsed")

        # Display the last model name input (disabled)
        st.markdown("Last Model Name")
        last_model_name_input = st.text_input("", st.session_state['last_model_name'], key="last_model_name_input", disabled=True, label_visibility="collapsed")
        ...
        if button_clicked:
            ...
            try:
                model_data = st.session_state['api_post_handler'].post(inference_model_endpoint, inference_model_data)
                st.info("Inference completed successfully!")
            except requests.exceptions.HTTPError as err:
                if err.response.status_code == 500:
                    st.error("Error 500: Internal Server Error occurred while performing inference.")
                else:
                    st.error(f"An error occurred: {err}")
            ...
            # fetch the s3 path
            model_inferences_data = st.session_state['api_get_handler'].get(model_inference_endpoint, model_inference_data)
            s3_path = model_inferences_data[0]['predictions']

            inference_data_local_path = S3Handler().download_file(s3_path, artifact_dir) # f'inference_{request.model_id}_{request.pred_date_id}.csv'
            ...
            try:
                st.session_state['inference_data'] = pd.read_csv(inference_data_local_path)
        ...
        if inference_data is not None and 'stock_id' in inference_data.columns:
            ...
            try:
                if st.session_state['stock_id_display'].strip():
                    stock_id_int = int(st.session_state['stock_id_display'])  # Ensure the ID is integer
                    inference_data_filtered = inference_data[inference_data['stock_id'] == stock_id_int]

                    if not inference_data_filtered.empty:
                        with tab1:
                            if not inference_data_filtered.empty:
                                st.dataframe(inference_data_filtered, use_container_width=True)
                            else:
    ...
    with show_data_tab:
        ...
        stock_id_input = st.text_input(
            "Enter the stock ID for plotting",
            value="0"
        )
        
        # Input for chart start date
        chart_date_id = st.text_input(
            f"Enter the date id for plotting (range from 1-480)",
            value="1"
        )
        ...
        if date_id is not None:
            if date_id > 480:
                ...
                if show_data_button_clicked:
                    ...
                    print(query_params)
                    stock_data_result = st.session_state['api_get_handler'].get(api_url = '/stock_data/', params=query_params)
                    ...
                    meaningful_timestamps = df_filtered['row_id'].apply(lambda x: convert_to_meaningful_time(x, holidays=st.session_state['holiday_dates_2023'])).tolist()
