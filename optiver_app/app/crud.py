...
def calculate_date_value(db: Session, date_id: int) -> date:
    ...
    if recent_date_mapping:
        ...
        logger.info(f"No recent date mapping found. Calculated base date: {calculated_date}.")
        return calculated_date
...
def get_or_create_date(db: Session, date_id: int) -> DateMapping:
    ...
    if instance is None:
        ...
        db.add(instance)
        db.commit()
        logger.info(f"Created new date mapping for date_id: {date_id}, date: {date_value}.")
    else:
        logger.info(f"Found existing date mapping for date_id: {date_id}, date: {instance.date}.")
