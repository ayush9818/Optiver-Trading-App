...
def get_or_create_date(args, db, date_id, date_value):
    ...
    if date_mapping is None:
        ...
        if args.commit:
            db.commit()
            logger.info(f"New date mapping created and committed for date_id: {date_id}.")
    return date_mapping
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-path", type=str, required=True, help="Path to the CSV file containing the data.")
    parser.add_argument("--batch-size", type=int, default=500, help="Number of rows to process in each batch.")
    parser.add_argument("--commit", action='store_true', help="If provided, commit the data to the database.")
